...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="tiktok-to-text", description="TikTok URL -> transcript (Whisper)"
    )
    p.add_argument("--url", required=True, help="URL del video TikTok")
    p.add_argument(
        "--out", default="outputs", help="Directorio de salida (default: outputs)"
    )
    p.add_argument(
        "--model", default="small", help="Modelo Whisper (tiny/base/small/medium/large)"
    )
    p.add_argument("--lang", default="es", help="Idioma (default: es)")
    p.add_argument(
        "--keep-video", action="store_true", help="No borrar el video descargado"
    )
